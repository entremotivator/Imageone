...
def load_image_with_fallback(file_info):
    """Try to load image from multiple sources with fallback logic."""
    ...
    # For AI-generated images, prioritize original URL (best quality)
    # For uploaded images, use Google Drive URLs
    if original_url:
        urls_to_try = [original_url, public_image_url, direct_link, thumbnail_url]
    else:
        urls_to_try = [public_image_url, direct_link, thumbnail_url]
        ...
        if url:
            try:
                ...
                if response.status_code == 200:
                    return PILImage.open(io.BytesIO(response.content)), url
            except Exception:
                continue
...
def display_library_page():
    ...
    if st.session_state.current_library_page >= total_pages and total_pages > 0:
        st.session_state.current_library_page = total_pages - 1
    
    start_idx = st.session_state.current_library_page * images_per_page
    ...
    with col2:
        if total_pages > 1:
            ...
            with page_cols[0]:
                if st.button("⬅️ Previous", disabled=(st.session_state.current_library_page == 0), use_container_width=True):
                    st.session_state.current_library_page -= 1
                    st.rerun()
            with page_cols[1]:
                st.markdown(f"<p style='text-align:center;margin-top:8px;'>Page {st.session_state.current_library_page + 1} of {total_pages}</p>", unsafe_allow_html=True)
            with page_cols[2]:
                if st.button("Next ➡️", disabled=(st.session_state.current_library_page >= total_pages - 1), use_container_width=True):
                    st.session_state.current_library_page += 1
                    st.rerun()
    
    with col3:
        if st.button("🔝 Go to First Page", disabled=(st.session_state.current_library_page == 0), use_container_width=True):
            st.session_state.current_library_page = 0
            st.rerun()
    ...
    if st.session_state.library_view_mode == 'grid':
        ...
        for i, file_info in enumerate(page_images):
            if i % cols_per_row == 0:
            ...
            with cols[i % cols_per_row]:
                ...
                with st.container():
                    ...
                    # Load and display image with Streamlit
                    with st.spinner(""):
                        img, loaded_url = load_image_with_fallback(file_info)
                        
                        if img:
                            # Display image with Streamlit's native image viewer
                    ...
                    st.markdown(f"**{file_name[:30]}{'...' if len(file_name) > 30 else ''}**")
                    ...
                    if created_time:
                        try:
                            ...
                            st.markdown(f"<span class='metadata-badge'>📅 {created_date.strftime('%m/%d %H:%M')}</span>", unsafe_allow_html=True)
                        except:
                            pass
                    ...
                    with edit_col1:
                        if st.button("✏️ Edit (Qwen)", key=f"edit_qwen_{file_id}_{start_idx + i}", use_container_width=True):
                    ...
                    with edit_col2:
                        if st.button("🎨 Edit (Seedream)", key=f"edit_seedream_{file_id}_{start_idx + i}", use_container_width=True):
                            st.session_state.selected_image_for_edit = file_info
                    ...
                    with btn_col1:
                        st.markdown(f"<a href='{web_link}' target='_blank' style='text-decoration:none;'><button style='width:100%;padding:8px;background:#4285F4;color:white;border:none;border-radius:6px;cursor:pointer;font-size:12px;'>🔗 Drive</button></a>", unsafe_allow_html=True)
                    
                    with btn_col2:
                        if img:
                            # Download button for loaded image
                            ...
                            st.download_button(
                                ...
                                file_name=file_name,
                                mime="image/png",
                                key=f"download_{file_id}_{start_idx + i}",
                                use_container_width=True
                            )
                    
                    with btn_col3:
                        if st.button("🗑️", key=f"delete_{file_id}_{start_idx + i}", use_container_width=True, help="Delete this image"):
                            if st.checkbox("Confirm?", key=f"confirm_del_{file_id}_{start_idx + i}"):
        ...
        for i, file_info in enumerate(page_images):
            ...
            with st.container():
                ...
                with col1:
                    # Load and display thumbnail with Streamlit
                    with st.spinner(""):
                        img, loaded_url = load_image_with_fallback(file_info)
                        ...
                        if img:
                            ...
                            st.markdown("🖼️ No preview available")
                ...
                with col2:
                    ...
                    if created_time:
                        try:
                            created_date = datetime.fromisoformat(created_time.replace('Z', '+00:00'))
                            metadata_parts.append(f"📅 {created_date.strftime('%Y-%m-%d %H:%M')}")
                        except:
                            pass
                    ...
                    if file_size:
                        try:
                            ...
                            if size_kb > 1024:
                                ...
                                size_str = f"{size_kb:.1f} KB"
                            metadata_parts.append(f"💾 {size_str}")
                        except:
                            pass
                    
                    if mime_type:
                        file_ext = mime_type.split('/')[-1].upper()
                        metadata_parts.append(f"📄 {file_ext}")
                    
                    if metadata_parts:
                        st.markdown(" | ".join(metadata_parts))
                    
                    # Expandable full-size view in list mode
                    if img:
                        with st.expander("🔍 View Full Size"):
                            st.image(img, caption=file_name)
                    ...
                    with btn_col1:
                        if st.button("✏️ Qwen", key=f"list_edit_qwen_{file_id}_{start_idx + i}", use_container_width=True):
                            st.session_state.selected_image_for_edit = file_info
                            st.session_state.edit_mode = 'qwen'
                            st.session_state.current_page = "Generate"
                            st.rerun()
                    ...
                    with btn_col2:
                        if st.button("🎨 Seedream", key=f"list_edit_seedream_{file_id}_{start_idx + i}", use_container_width=True):
                            st.session_state.selected_image_for_edit = file_info
                            st.session_state.edit_mode = 'seedream'
                            st.session_state.current_page = "Generate"
                            st.rerun()
                    ...
                    with btn_col4:
                        if img:
                            ...
                            st.download_button(
                                ...
                                file_name=file_name,
                                mime="image/png",
                                key=f"list_download_{file_id}_{start_idx + i}",
                                use_container_width=True
                            )
                    
                    with btn_col5:
                        if st.button("🗑️", key=f"list_delete_{file_id}_{start_idx + i}", use_container_width=True, help="Delete"):
                            if st.checkbox("Confirm?", key=f"list_confirm_del_{file_id}_{start_idx + i}"):
    ...
    if total_pages > 1:
        ...
        with col1:
            if st.button("⬅️ Previous Page", disabled=(st.session_state.current_library_page == 0), use_container_width=True, key="bottom_prev"):
                st.session_state.current_library_page -= 1
                st.rerun()
        with col2:
            st.markdown(f"<p style='text-align:center;margin-top:8px;font-weight:bold;'>Page {st.session_state.current_library_page + 1} of {total_pages}</p>", unsafe_allow_html=True)
        with col3:
            if st.button("Next Page ➡️", disabled=(st.session_state.current_library_page >= total_pages - 1), use_container_width=True, key="bottom_next"):
                st.session_state.current_library_page += 1
                st.rerun()
