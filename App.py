...
# <CHANGE> Load secrets from Streamlit secrets
def load_secrets():
    """Load API key and service account from Streamlit secrets."""
    api_key = None
    service_account_json = None
    
    try:
        # Try to get API key from secrets
        api_key = st.secrets.get("KIE_API_KEY", None)
        
        # Try to get Google service account credentials from secrets
        if "GOOGLE_SERVICE_ACCOUNT" in st.secrets:
            service_account_json = dict(st.secrets["GOOGLE_SERVICE_ACCOUNT"])
        
    except Exception:
        pass
    
    return api_key, service_account_json

API_KEY_FROM_SECRETS, SERVICE_ACCOUNT_FROM_SECRETS = load_secrets()
...
def init_session_state():
    ...
    defaults = {
        'api_key': API_KEY_FROM_SECRETS if API_KEY_FROM_SECRETS else "",
        'task_history': [],
        'current_task': None,
        'authenticated': False,
        'service': None,
...
def display_gdrive_image(file_info, width='stretch', caption=None):
    """Robustly display an image from Google Drive."""
    ...
    if image_bytes:
        try:
            st.image(image_bytes, width=width, caption=caption or file_name)
            return True
        except Exception as e:
            st.warning(f"Could not display {file_name}: {str(e)}")
    ...
    <div style='padding: 20px; background: #f0f0f0; border-radius: 8px; text-align: center;'>
        ...
        <a href='{file_info.get('webViewLink', '#')}' target='_blank'>Open in Drive</a>
    </div>
    """, unsafe_allow_html=True)
    return False
...
# <CHANGE> Load service account from secrets or session state on startup
def load_persisted_service_account():
    """Load service account from secrets or session state."""
    if SERVICE_ACCOUNT_FROM_SECRETS and not st.session_state.authenticated:
        try:
            authenticate_with_service_account(SERVICE_ACCOUNT_FROM_SECRETS)
            create_app_folder()
        except Exception as e:
            st.warning(f"Failed to auto-authenticate with Google Drive: {str(e)}")
    elif st.session_state.service_account_info and not st.session_state.authenticated:
        try:
...
with st.sidebar:
    ...
    # <CHANGE> Show different UI based on whether secrets are configured
    if API_KEY_FROM_SECRETS:
        st.success("✅ API Key loaded from secrets")
        st.code(f"{API_KEY_FROM_SECRETS[:8]}...{API_KEY_FROM_SECRETS[-4:]}", language=None)
    else:
        api_key_input = st.text_input(
            ...
            help="Enter your KIE.AI API key (or configure in secrets.toml)"
        )
    ...
    if not st.session_state.authenticated:
        # <CHANGE> Show different UI based on whether secrets are configured
        if SERVICE_ACCOUNT_FROM_SECRETS:
            st.info("🔄 Service account loaded from secrets")
        else:
            ...
            uploaded_file = st.file_uploader(
                ...
                help="Upload your Google service account credentials (or configure in secrets.toml)"
            )
        ...
        if st.button("🗑️ Disconnect", key="sidebar_disconnect"):
            st.session_state.authenticated = False
            st.session_state.service = None
            st.session_state.credentials = None
            if not SERVICE_ACCOUNT_FROM_SECRETS:
                st.session_state.service_account_info = None
            st.session_state.needs_rerun = True
    ...
    if st.button("📋 View All Tasks", key="sidebar_view_tasks"):
        st.session_state.current_page = "History"
        st.session_state.needs_rerun = True
    
    if st.button("📚 Open Library", key="sidebar_open_library"):
        st.session_state.current_page = "Library"
        st.session_state.needs_rerun = True
    
    if st.button("🗑️ Clear History", key="sidebar_clear_history"):
...
def display_generate_page():
    ...
    if not st.session_state.api_key:
        st.error("Please configure your API Key in the sidebar or in secrets.toml to start generating images.")
        
        # <CHANGE> Show instructions for configuring secrets
        with st.expander("📝 How to configure Streamlit secrets"):
            st.markdown("""
            ...
            3. The file will be automatically loaded when you run the app
            
            **Note:** The `.streamlit/secrets.toml` file should be added to `.gitignore` to keep your secrets safe!
            """)
        return
        ...
        with st.form("qwen_image_edit_form"):
            ...
            if st.session_state.authenticated and st.session_state.library_images:
                ...
                if use_library_image:
                    ...
                    display_gdrive_image(selected_img, width=None)
                else:
    ...
    with tab4:
        ...
        if not st.session_state.authenticated:
            ...
            if uploaded_files:
                ...
                if st.button("⬆️ Upload All to Google Drive", type="primary", key="upload_all_btn"):
                    progress_bar = st.progress(0)
                    status_text = st.empty()
                    ...
                    if success_count > 0:
                        if st.button("📚 View in Library", key="view_lib_after_upload"):
                            st.session_state.current_page = "Library"
                            st.session_state.needs_rerun = True
            else:
                ...
                with cols[j]:
                    ...
                    if st.session_state.authenticated:
                        ...
                        if not is_uploaded:
                            ...
                            if st.button("⬆️ Upload to Drive", key=upload_key):
...
def display_library_page():
    ...
    with col1:
        if st.button("⬅️ Back to Generate", key="lib_back_btn"):
    ...
    if not st.session_state.library_images:
        ...
        if st.button("📤 Go to Upload Tab", key="lib_goto_upload"):
            st.session_state.current_page = "Generate"
            st.session_state.needs_rerun = True
        return
        ...
        for i, file_info in enumerate(filtered_images):
            ...
            with cols[i % cols_per_row]:
                ...
                with st.container():
                    ...
                    display_gdrive_image(file_info, width=None)
                    ...
                    with edit_col1:
                        if st.button("✏️ Edit (Qwen)", key=f"edit_qwen_{file_id}"):
                    ...
                    with edit_col2:
                        if st.button("🎨 Edit (Seedream)", key=f"edit_seedream_{file_id}"):
                            st.session_state.selected_image_for_edit = file_info
                    ...
                    with btn_col2:
                        st.markdown(f"<a href='{web_link}' target='_blank' style='text-decoration:none;'><button style='width:100%;padding:8px;background:#34A853;color:white;border:none;border-radius:6px;cursor:pointer;'>👁️ View</button></a>", unsafe_allow_html=True)
                    
                    with btn_col3:
                        if st.button("🗑️", key=f"delete_{file_id}", help="Delete this image"):
        ...
        for i, file_info in enumerate(filtered_images):
            ...
            with st.container():
                ...
                with col2:
                    ...
                    with btn_col1:
                        if st.button("✏️ Qwen", key=f"list_edit_qwen_{file_id}"):
                            st.session_state.selected_image_for_edit = file_info
                            st.session_state.edit_mode = 'qwen'
                            st.session_state.current_page = "Generate"
                            st.session_state.needs_rerun = True
                    ...
                    with btn_col2:
                        if st.button("🎨 Seedream", key=f"list_edit_seedream_{file_id}"):
                            st.session_state.selected_image_for_edit = file_info
                            st.session_state.edit_mode = 'seedream'
                            st.session_state.current_page = "Generate"
                            st.session_state.needs_rerun = True
...
# Handle reruns outside of callbacks
